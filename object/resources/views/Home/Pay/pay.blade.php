...
<html>
	<head>
		...
		<link href="/Home/shopping/css/jsstyle.css" rel="stylesheet" type="text/css">
		<script type="text/javascript" src="/Home/shopping/js/address.js"></script>
		<link rel="stylesheet" href="/Home/shopping/css1/style.css">
		<script type="text/javascript" src="/Home/shopping/js1/demo.js"></script>
		<!-- 听云监控 -->
	...
	<body>
    			...
    			<div class="cart_top">
        			<div class="user_box" id="JS_user_box">
        				<div>聚美优品欢迎您，<a href="{{ asset('home/login/index') }}">登录</a>
        					<i class="tips">|</i>
        					<a href="{{ asset('home/register') }}a">免费注册</a>
        				</div>
        			</div>
    			</div>
   			@endif
		...
		<div id="container" style="width: auto;"><script type="text/javascript">
		window.GLOBAL =  {
		...
		<form action="/home/pay" method='post' name='myform' id='form1'>
			{{ csrf_field() }}
			{{ method_field('DELETE') }}
		</form>
		<form action="/home/pay/bindaddress" method="post">
			{{ csrf_field() }}
			<!--订单支付页 -->
			<div class="catbox">
				<h3 class="block-title" style='font-size:20px;background:#eaeaea;'>支付页面</h3>
				<table id="cartTable">
					<thead>
				    	<tr>
					    	<th>
					...
					<tbody>
						...
						@foreach ($list as $v)
				      		<tr>
				      			<td>
				      				<input class="check-one check" name="oid[]" type="checkbox" value="{{ $v->oid }}"/>
				      			</td>
				        		<td class="goods">
				        			<img src="/Admin/upload/{{ $v->pic }}" alt=""/><span>{{ $v->pname}}</span>
				        		</td>
				        		<td class="price">{{ $v->money }}</td>
				        		<td class="count"><span class="reduce"></span>
				          			<input class="count-input" type="text" value="{{ $v->sum }}"/>
				          			<span class="add">+</span>
				          		</td>
				        		<td class="subtotal">{{ $v->money * $v->sum }}</td>
				        		<td class="operation">
				        			<a href="javascript:doDel({{ $v->oid }})" data-point-url="#" class="delete">取消订单</a>
				        		</td>
				      		</tr>
				   		@endforeach
						@else
					...
					<script type="text/javascript">
				    	function doDel(oid){
				     		if(confirm('确定取消订单吗？')){
				         		...
				         		form.action = '/home/pay/'+oid;
				         		form.submit();
				     		}
				  		}
					</script>
				</table>
				<!-- <div class="catbox" style='height:10px;'>
					{{ $list->links() }}
				</div> -->
			  	<div class="foot" id="foot">
			  		...
			  		<div class="fr closing" onclick="getTotal();">
						<a href="{{ asset('/home/shopcart')}}">返回购物车</a>
					</div>
			  	...
			  	<table width='800' border='1'>		
					...
					<tr>
						<td>
							<input type='radio' name='addr_id[]' value="{{ $v->id }}">
						</td>
						<td>{{ $v->aname }}</td>
						<td>{{ $v->uphone }}</td>
						<td>{{ $v->uaddress }}</td>
				...
				<div id="payTable">
					...
					<div id="holyshit269" class="submitOrder">
						<div class="go-btn-wrap">
							<a href="/home/sign/addr" class="btn-go" tabindex="0" title="">请输入地址</a>
						</div>
					</div>
					<!--信息 -->
					<div class="order-go clearfix">
						<div class="pay-confirm clearfix">
							<div id="holyshit269" class="submitOrder">
								<div class="go-btn-wrap">
									<!--<a id="J_Go" href="{{ url('/home/success') }}" class="btn-go" tabindex="0" title="点击此按钮，提交付款">确认付款</a> -->
									<input type="submit" name="do" value="确认付款" class="btn-go" style="float:right;">
								</div>
							</div>
							<div class="clear"></div>
						</div>
